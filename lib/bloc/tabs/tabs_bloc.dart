...
import 'package:mobile/bloc/tabs/tabs_state.dart';
...
class TabsBloc extends Bloc<TabsEvent, TabsState> {
  ...
  TabsBloc({required this.connectivityBloc}) : super(const TabsStateLoading()) {
    ...
    on<TabMenuOpenEvent>((event, emit) =>
        _tabMenuOpenEvent(event, emit, event.index));
  ...
  void _changeTabOrderEvent(TabsEvent event, Emitter<TabsState> emit, int oldIndex, int newIndex) async {
    try {
  ...
  void _createTabEvent(TabsEvent event, Emitter<TabsState> emit, MyTab tabFormData) async {
    try {
  ...
  void _updateTabEvent(TabsEvent event, Emitter<TabsState> emit, int index, MyTab tabFormData) async {
    try {
      ...
      if (isUpdated) {
        bool isUpdatedLocal = await dataRepository.updateTabs(index, tabFormData);
        if (isUpdatedLocal) {
          emit(const TabsStateLoading(mustRebuild: true));
        } else {
          emit(const TabsStateFailure(tabsFailureMessage: "tab couldn't updated in locale"));
        }
      } else {
        emit(
          const TabsStateFailure(
  ...
  void _deleteTabEvent(TabsEvent event, Emitter<TabsState> emit, int index) async {
    try {
      MyTab? victimTab = dataRepository.tabsBox.getAt(index);
      String? accessToken = await dataRepository.getAccessToken();
      bool isDeleted = await noteApiService.fetchDeleteTab(accessToken: accessToken, id: victimTab?.id);
  ...
  void _tabMenuOpenEvent(TabsEvent event, Emitter<TabsState> emit, int index) async {
  }
}
